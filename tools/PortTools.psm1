...
function Update-Baseline {
    ...
    # -CaseSensitive: without it Sort-Object -Unique collapses two ids that differ only in case,
    # which is exactly what a parameterised test of case-insensitive behaviour produces, and the
    # baseline silently stops covering one of them. Ordinal sorting also keeps the diff stable
    # across machines with different cultures.
    $passing = @($Results | Where-Object Outcome -eq 'Passed' | ForEach-Object Id |
