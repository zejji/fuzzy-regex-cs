...
public sealed class CapturesTests
{
    ...
    {
        Match m = Upstream.Match("abcdef", @"(\w{3})+");
    ...
    {
        Match m = Upstream.Match("192.168.0.1", @"^(\d{1,3})(?:\.(\d{1,3})){3}$");
    ...
    {
        Match m = Upstream.MatchAtStart("3FB52A0C a2c4g3k9d3", @"^([0-9A-F]{2}){4} ([a-z]\d){5}$");
    ...
    {
        Match m = Upstream.MatchAtStart("aWbXcXdXeXfY", "([a-z]W)([a-z]X)+([a-z]Y)");
    ...
    {
        Match m = Upstream.Match("abcd", @".*?(?>(.){0,2})d");
