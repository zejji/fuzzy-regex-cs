...
public sealed class PossessiveTests
{
    ...
    {
        Match m = Upstream.Match("aaa", "a*a");
    ...
    {
        Match m = Upstream.Match("aaa", "a{1,3}a");
    ...
    {
        Match m = Upstream.Match("ab", "(?:ab)?ab");
    ...
    {
        Match m = Upstream.Match("ababab", "(?:ab)*ab");
    ...
    {
        Match m = Upstream.Match("ababab", "(?:ab){1,3}ab");
    ...
    public void Possessive_bounded_repeat_group_then_group_does_not_backtrack() =>
        Upstream.Match("ababab", "(?:ab){1,3}+ab").Success.Should().BeFalse();
}
