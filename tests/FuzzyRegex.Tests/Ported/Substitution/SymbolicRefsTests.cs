...
public sealed class SymbolicRefsTests
{
    ...
    {
        Action act = static () => _ = Upstream.Replace("xx", "(?P<a>x)", @"\g<a");
    ...
    {
        Action act = static () => _ = Upstream.Replace("xx", "(?P<a>x)", @"\g<");
    ...
    {
        Action act = () => _ = Upstream.Replace("xx", "(?P<a>x)", replacement);
    ...
    {
        Action act = static () => _ = Upstream.Replace("xx", "(?P<a>x)", @"\g<ab>");
    ...
    public void Replace_with_a_named_reference_to_a_group_that_did_not_participate_expands_to_empty() =>
        Upstream.Replace("xx", "(?P<a>x)|(?P<b>y)", @"\g<b>").Should().Be("");
