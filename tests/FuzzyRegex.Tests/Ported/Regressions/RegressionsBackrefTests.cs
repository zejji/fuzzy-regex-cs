...
public sealed class RegressionsBackrefTests
{
    ...
    {
        Match m = FuzzyRegex.Match("b", @"^(a|)\1{2}b");
    ...
    {
        Match m = FuzzyRegex.Match("xx", @"(?V1)(\1xx|){6}");
    ...
    {
        Match m = FuzzyRegex.MatchAtStart("x  |y| z|", @"x.*? (.).*\1(.*)\1");
    ...
    {
        Match m = FuzzyRegex.MatchAtStart(".sr  h |<nw>|<span class=\"locked\">|", @"\.sr (.*?) (.)(.*)\2(.*)\2(.*)");
    ...
    {
        Match m = FuzzyRegex.Match("abca", @"^a?(a?)b?c\1$");
