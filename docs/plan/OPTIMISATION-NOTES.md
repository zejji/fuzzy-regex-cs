...
| `Engine/Iteration.cs:216` | One `MatchState` per `NextMatch` step | A walk re-creates state per match. After S53b, `EnumerateMatches` inherits this; the lift is one state reused across the walk. |
| `MatchCollections.cs:115` | `Matches` is eager | Stays eager by design (`Count` promise); S53b adds the lazy entry points. |
