...
| `Engine/FuzzyLiteralFilter.cs` | `basic_match` (`_regex.c:11767-11814`) runs no prefilter under a fuzzy section: the required-string search is off, because an error can delete the character it would look for, so every start position is attempted | A pattern that is exactly one fuzzy section over an ASCII literal, an alternation of them or a named list, with a bounded budget `k`: each literal is cut into `k + 1` pieces (Navarro 2001, section 8.1). Before each forward attempt, `Matcher.BasicMatch` moves the start to the earliest position an untouched piece allows, or refuses the subject when no piece occurs; a reverse search only refuses. Off for partial matching and for a non-ASCII stretch of subject | `ManyInputs` `FuzzyPhraseOneIsMatch` **2.424 s to 64.2 ms**, `FuzzyPhraseThreeSeparatePasses` 7.438 s to 164.9 ms, allocation 92.39 MB to 87.18 MB; `FuzzyPhraseThreeAlternation` 6.798 s to 171.7 ms, `FuzzyPhraseThreeNamedList` 6.935 s to 168.0 ms, 92.5 MB to 87.5 MB (2026-09-23, Release, `--inProcess`, back to back on this machine with other sessions idle) | Nothing to follow unless upstream adds a fuzzy prefilter of its own, or changes what a fuzzy section can match: an error that damages two characters at once would break the one-edit-one-piece argument in the type's remarks | S60b, 2026-09-23 |
| `Engine/Matcher.cs` (`SameCharIgn`) | `same_char_ign` (`_regex.c:2849`) always calls the encoding's `all_cases` for the first character and scans the result | When both characters are ASCII and the encoding is ASCII or Unicode, the answer is the ASCII case rule: the two cases of one letter. `SameCharIgnTests` proves it equal to the enumeration over every ASCII pair under every encoding-flag combination | About **9% on a fuzzy and 17% on a plain IGNORECASE search** the fuzzy literal filter does not reach. Stopwatch probe, Release, 200,000 characters of ASCII words, median of 9, two alternating runs each: `(?:hay\w?stack){e<=2}` `Count` 129.2 / 132.6 ms before, 119.2 / 120.0 ms after; `qu\w+ck\s+b` 6.3 / 6.0 ms before, 5.0 / 5.2 ms after; `(\w{3}) \1` unchanged at 20.5 ms (2026-09-23, alternating runs beside another session's benchmark). On `ManyInputs` `FuzzyPhraseOne*` it is inside the error bars, because the filter skips most start positions there | If upstream changes `all_cases` for an ASCII codepoint, or this port changes `TurkicDefaults`, re-run `SameCharIgnTests` | S60b, 2026-09-23 |
