...
| `Engine/Matcher.cs` (`SameCharIgn`) | `same_char_ign` (`_regex.c:2849`) always calls the encoding's `all_cases` for the first character and scans the result | When both characters are ASCII and the encoding is ASCII or Unicode, the answer is the ASCII case rule: the two cases of one letter. `SameCharIgnTests` proves it equal to the enumeration over every ASCII pair under every encoding-flag combination | **Not yet shown.** The ce4ca5a profile put `Encodings.AllCases` under this call at 9.7% exclusive of an IGNORECASE fuzzy search. Back to back on `ManyInputs` `FuzzyPhraseOne*` (Release, `--inProcess`), after 66.6 / 61.8 / 64.3 ms against before 63.3 / 63.0 / 65.9 ms, within the error bars, with another session's MediumRun benchmark on the machine. Lands with its time gate open, per the orchestrator's rule for gains under 2x; re-measure on a quiet machine | If upstream changes `all_cases` for an ASCII codepoint, or this port changes `TurkicDefaults`, re-run `SameCharIgnTests`. If the quiet re-measure shows no gain, delete the fast path and this row | S60b, 2026-09-23 |
