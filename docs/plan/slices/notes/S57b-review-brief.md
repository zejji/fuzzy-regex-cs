Review the uncommitted working tree of C:/Users/gerard.howell/source/repos/fuzzy-regex-cs for
DEFECTS only. Scope: `git diff` plus the untracked files under `tools/probes/` (run
`git status --porcelain`). It is one sitting of slice S57b, which judges the last six red rows of a
6000-row differential-oracle gate against Python `regex 2026.9.10`.
...
- New this sitting: entries `leaked-changes-beside-a-truncated-partial`,
...
- ALSO NEW, and judged the same way: the gate was re-recorded at `-Count 6000` and its third default
  ...
  every diverging row. Rows 72790 and 74947 are deliberately NOT pinned and the notes say why; that
  the gate is still red on those two is not a finding, but a wrong reason in the notes is.
...
RULES
- You are working on a tree with UNCOMMITTED changes and there is NO commit to fall back to. NEVER
...
- Do NOT run `pwsh -File tools/run-oracle.ps1` without `-Rows` or `-SkipRecord`: a bare run
  ...
  `TestResults/oracle/wave.jsonl`, which currently holds seed 20260921, and is the cheap way to see
  the pins classify (expect `diverge 2`, rows 72790 and 74947).
