...
- [x] Every divergence judged, fixed or entered with a control; nothing unjudged.
      (Sitting 19 judged the last 22 in one pass, from an ablation matrix rather than row by row:
      `pwsh -File tools/run-oracle.ps1 -Rows tools/probes/sweep-divergence-rows.jsonl` is
      **expected 37, diverge 0**. The gate's 104366 is S52c/S52d's by the owner's ruling.)
- [x] Ratchet GREEN, blind review (hunt: an astral row whose index is converted twice; a long-subject
      generator that never reaches the path it was written for), commit.
      (Sitting 19: ratchet GREEN, 6127 / 6127 / 0 skipped, 6019 distinct ids, baseline 6019.)
---

## Sitting notes

Every sitting's notes live in `docs/plan/slices/notes/S52-sittings.md`, moved there verbatim on
2026-09-16 so that this file stays readable as a spec. The closing notes for the slice go there
too; what stays here is the scope, the verification and the boxes above.
