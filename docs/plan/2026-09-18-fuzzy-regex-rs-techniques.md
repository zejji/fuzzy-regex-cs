...
| 5 | Reverse search for `.*SUFFIX` (compile/search) | impl_overview §3: "`.*test` uses reverse search (O(n) vs O(n^2))" | Unclear: no evidence it preserves capture positions and leftmost order for patterns with groups. Not adopted; revisit only with a proof |
