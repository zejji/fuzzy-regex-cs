...
| `_check_group_features` | `_regex_core.py:4421-4458` | `Parsing.ParseFunctions.CheckGroupFeatures` - **implemented in S11**. The `isinstance(parsed, Fuzzy)` test is still `false`; the `Fuzzy` node is S13's | S11 |
