...
| `{e<=1:\X}`, `{e<=1:\b}`, `{e<=1:\A}`, `{e<=1:\Z}`, `{e<=1:\L<a>}` - a zero-width or multi-character node as a fuzzy test | Upstream's *parser* accepts all five and emits exactly the bytecode this port emits, word for word; its *C compiler* then answers `RuntimeError: invalid RE code`. This port compiles them. | The rejection is the C engine's opcode validator, which is the `re_compile` port (`_regex.c:25863-26121`) and therefore Phase 3's, not the parser's. Adding a validator to `PatternCompiler` would be a second implementation of it in the wrong layer. Fourteen rows of the S13 wave; **the Phase 3 slice that ports `re_compile` must reject these**, and the wave script is the ready-made check. Contrast the `\1` row above, where the divergence is not deferrable because our `uint` code list cannot hold upstream's value at all. | S13 |
