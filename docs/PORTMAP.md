...
| `push_int8`, `push_code`, `push_int`, `push_groups`, `push_captures`, `push_guard_data`, `push_repeat_data`, `push_repeats` and their pops | `:2434-2815` | **Not ported yet - they arrive with their first caller.** `push_groups`/`pop_groups` and `push_captures`/`pop_captures` are called only from `ATOMIC` (`:12040`), `CONDITIONAL` (`:12226`), `LOOKAROUND` (`:13774`), `GROUP_CALL` (`:13405`) and `GROUP_RETURN` (`:13500`), all Phase 4; the guard and repeat pushes are S19's |
...
| `reset_guards` | `:3383` | **Not ported yet.** No repeat guards until S19, no fuzzy guards until Phase 5, no group-call guards until Phase 4; a pattern needing any of them throws at its own opcode first. Both call sites carry a comment naming it |
...
| `match_many_ALL`, `match_many_ANY`, `match_many_*` for the class opcodes | `:4045`, `:4485`, `:4737` | **Not ported yet - S19.** They are bulk variants whose only callers upstream are the repeat opcodes, and the S16/S17 spine steps one character at a time |
...
| `try_match` (the test-node fast path) | `:7671` | `Matcher.TryMatch`, **reduced to its default arm** (`:7842`): it reports success and leaves the position alone, so `BRANCH` enters the alternative and the dispatch loop tests its first node in the ordinary way. Semantically transparent - where upstream would refuse the branch, this port enters it and backtracks straight out again. The predicate dispatch is Phase 7's |
