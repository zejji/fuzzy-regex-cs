...
| `locale_all_cases`, `locale_simple_case_fold`, `locale_full_case_fold`, `scan_locale_chars`, `locale_encoding` (`_regex.c:1057-1360`) | The `LOCALE` encoding reads the process's C locale, which has no .NET equivalent and is not surfaced on `FuzzyRegexOptions`. `Unicode.Encodings.Select` throws `needs:locale-flag` for it. What `(?L)` should do with a `str` pattern is settled against the oracle in S10. | S09 |
