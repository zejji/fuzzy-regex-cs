...
| `push_int8`, `push_code`, `push_int`, `push_groups`, `push_captures`, `push_guard_data`, `push_repeat_data`, `push_repeats` and their pops | `:2434-2815` | **Not ported yet - they arrive with their first caller, and none of them was S19's.** `push_groups`/`pop_groups` and `push_captures`/`pop_captures` are called only from `ATOMIC` (`:12040`), `CONDITIONAL` (`:12226`), `LOOKAROUND` (`:13774`), `GROUP_CALL` (`:13405`) and `GROUP_RETURN` (`:13500`), all Phase 4. The S19 slice file listed the guard and repeat pushes as its own; they are not - `grep -n 'push_repeats\|pop_repeats'` gives ten call sites (`:12228`, `:12431`, `:13407`, `:13502`, `:13514`, `:15423`, `:15475`, `:16369`, `:16400`, `:16416`) and every one is inside `CONDITIONAL`, `END_CONDITIONAL`, `GROUP_CALL` or `GROUP_RETURN`. The repeat opcodes push their own state through `RE_RepeatStateData` and friends instead. `push_code` is `PushSize` here, 8 bytes where upstream pushes 4 |
