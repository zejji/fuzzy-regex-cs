...
| Set-member order at `_check_firstset` | S06's PORTMAP rule says S07 and S08 must sort the first set's members. **S07 does not sort**, because it throws `needs:character-classes` at the line that would build the `SetUnion` instead. The sort lands with the set node. Measured: exactly one corpus row (`()ef`) is lost to this. | The sort only matters once the members are turned into an ordered list, which is the `SetUnion` construction itself. | S07 |
